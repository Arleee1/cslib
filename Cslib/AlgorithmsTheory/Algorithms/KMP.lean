...
private lemma innerLPSWhile_eval_lower_bound [BEq α]
  ...
  | succ fuel ih =>
      ...
      · have hnextPat : Int.toNat (table[(Int.toNat cnd)]'hcndTable) < pat.length := by
          exact htableBound (Int.toNat cnd) hcndTable
        have hnextTable : Int.toNat (table[(Int.toNat cnd)]'hcndTable) < table.length := by
          calc
            Int.toNat (table[(Int.toNat cnd)]'hcndTable) < pat.length := hnextPat
            _ < pat.length + 1 := Nat.lt_succ_self _
            _ = table.length := htableLen.symm
...
private lemma innerLPSWhile_eval_add_one_upper_bound [BEq α]
  ...
  have hbase : Int.toNat (cnd + 1) ≤ Int.toNat cnd + 1 :=
    int_toNat_add_one_le_of_ge_neg_one hcndLower
  induction fuel generalizing pos cnd with
  | zero =>
  ...
  | succ fuel ih =>
      ...
      · have hnextPat : Int.toNat (table[(Int.toNat cnd)]'hcndTable) < pat.length := by
        ...
        · have hstep := htableStep (Int.toNat cnd) hcndTable
          ...
          · have hnextEq : table[(Int.toNat cnd)]'hcndTable = -1 := by
              omega
            ...
            omega
          · have hnextNonneg : 0 ≤ table[(Int.toNat cnd)]'hcndTable := by
              omega
            have hstep' :
...
-- The nested case splits and Presburger arithmetic in this amortized proof are expensive.
private lemma LPSWhile_time_complexity_upper_bound [BEq α]
  ...
  | succ fuel ih =>
    ...
    · -- cmp=true, pos+1 < pat.length, cndPat' < pat.length: recurse
      have htb : ∀ (i : Nat) (hi : i < (table.set pos table[cnd.toNat]).length),
          (table.set pos table[cnd.toNat])[i].toNat < pat.length := by
        intro i hi; simp only [List.length_set] at hi
        by_cases heq : i = pos
        · subst heq; simp; exact htableBound _ hcndTable
        · rw [List.getElem_set_of_ne (by omega)]; exact htableBound i (by omega)
      have hlower : ∀ (i : Nat) (hi : i < (table.set pos table[cnd.toNat]).length),
          -1 ≤ (table.set pos table[cnd.toNat])[i]'hi := by
        intro i hi
        simp only [List.length_set] at hi
        by_cases heq : i = pos
        · subst heq
          simp
          exact htableLower _ hcndTable
        · rw [List.getElem_set_of_ne (by omega)]
          exact htableLower i (by omega)
      have hstep : Int.toNat (cnd + 1) = Int.toNat cnd + 1 := int_toNat_add_one_eq hcndNonneg
      have hcndStep' : Int.toNat ((cnd + 1) + 1) ≤ pos + 1 := by
        rw [int_toNat_add_one_eq (by omega : 0 ≤ cnd + 1), hstep]
        omega
      have hpred : ∀ (i : Nat) (hi : i < (table.set pos table[cnd.toNat]).length),
          Int.toNat ((table.set pos table[cnd.toNat])[i]'hi + 1) ≤ i := by
        intro i hi
        simp only [List.length_set] at hi
        by_cases heq : i = pos
        · subst heq
          simp
          have := htableStep (Int.toNat cnd) hcndTable
          rw [hstep] at hcndStep
          omega
        · rw [List.getElem_set_of_ne (by omega)]
          exact htableStep i (by omega)
      have hrec := ih (pos + 1) (cnd + 1) (table.set pos table[cnd.toNat]) hpos' hcndPat'
        (by simp [List.length_set]; omega) (by rw [List.length_set, htableLen]) htb
        (by omega) hcndStep' hlower hpred
      omega
    · simp [Prog.time_pure]
    · simp [Prog.time_pure]
    · -- cmp=false, pos' < pat.length: inner loop + possible recurse
      rename_i hpos_next
      rw [Prog.time_bind]
      ...
      · have htb2 : ∀ (i : Nat) (hi : i < (table.set pos cnd).length),
            (table.set pos cnd)[i].toNat < pat.length := by
          intro i hi; simp only [List.length_set] at hi
          by_cases heq : i = pos
          · subst heq; simp; exact hcndPat
          · rw [List.getElem_set_of_ne (by omega)]
            exact htableBound i (by omega)
        have hlower2 : ∀ (i : Nat) (hi : i < (table.set pos cnd).length),
            -1 ≤ (table.set pos cnd)[i]'hi := by
          intro i hi
          simp only [List.length_set] at hi
          by_cases heq : i = pos
          · subst heq
          ...
          · rw [List.getElem_set_of_ne (by omega)]
            exact htableLower i (by omega)
        have hpred2 : ∀ (i : Nat) (hi : i < (table.set pos cnd).length),
            Int.toNat ((table.set pos cnd)[i]'hi + 1) ≤ i := by
          intro i hi
          simp only [List.length_set] at hi
          by_cases heq : i = pos
          · subst heq
            simpa using hcndStep
          · rw [List.getElem_set_of_ne (by omega)]
            exact htableStep i (by omega)
        have hnextNonneg :
            0 ≤
        ...
        have hnextStep :
            Int.toNat
              (((innerLPSWhile table.length pos cnd pat table
                  hpos hcndPat hcndTable htableLen htableBound).eval
                  Comparison.natCost + 1) + 1) ≤ pos + 1 := by
          have hsucc :
              Int.toNat
                (((innerLPSWhile table.length pos cnd pat table
                    hpos hcndPat hcndTable htableLen htableBound).eval
                    Comparison.natCost + 1) + 1) =
                Int.toNat
                  ((innerLPSWhile table.length pos cnd pat table
                      hpos hcndPat hcndTable htableLen htableBound).eval
                      Comparison.natCost + 1) + 1 := by
            rw [int_toNat_add_one_eq hnextNonneg]
          have hcndSucc : Int.toNat (cnd + 1) = Int.toNat cnd + 1 :=
            int_toNat_add_one_eq hcndNonneg
          rw [hsucc, hcndSucc] at *
          omega
        have hrec := ih (pos + 1)
          ((innerLPSWhile table.length pos cnd pat table
            hpos hcndPat hcndTable htableLen htableBound).eval
            Comparison.natCost + 1)
          (table.set pos cnd) hpos_next hcndPat_rec
          (by simp [List.length_set]; omega)
          (by rw [List.length_set, htableLen]) htb2
          hnextNonneg hnextStep hlower2 hpred2
        omega
      · rw [Prog.time_pure]; simp
        have := hinner
        omega
    · -- cmp=false, ¬pos' < pat.length
      rw [Prog.time_bind, Prog.time_pure]
      have hinner := innerLPSWhile_time_eval_upper_bound
        table.length pos cnd pat table
        hpos hcndPat hcndTable htableLen htableBound htableLower htableStep
      simp
      omega
...
theorem buildLPS_time_complexity_upper_bound [BEq α]
  ...
  have hlower :
      ∀ (i : Nat) (hi : i < (-1 :: List.replicate (List.length (x :: y :: xs)) 0).length),
      -1 ≤ (-1 :: List.replicate (List.length (x :: y :: xs)) 0)[i]'hi := by
    intro i hi
    cases i <;> simp
  have hstep :
      ∀ (i : Nat) (hi : i < (-1 :: List.replicate (List.length (x :: y :: xs)) 0).length),
      Int.toNat (((-1 :: List.replicate (List.length (x :: y :: xs)) 0)[i]'hi) + 1) ≤ i := by
    intro i hi
    cases i <;> simp
